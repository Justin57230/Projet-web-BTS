...
-- Table des abonnemment
CREATE TABLE abonnements (
    id_abonnement INT AUTO_INCREMENT PRIMARY KEY,
    id_abonne INT,
    id_abonnement_a INT,
    FOREIGN KEY (id_abonne) REFERENCES utilisateurs(id_users),
    FOREIGN KEY (id_abonnement_a) REFERENCES utilisateurs(id_users),
    CONSTRAINT unique_abonnement UNIQUE (id_abonne, id_abonnement_a)
);


-- Définition de la requête SQL pour mettre à jour le chemin de la photo
UPDATE utilisateurs 
SET photo_profil = :nouveauChemin
WHERE idUtilisateur = :id_users;
